...
iOS demo assets — powered by **[ModelRunner](https://modelrunner.run)**, the unified AI inference
API. Authored once as [`SKILL.md`](https://modelrunner.run) and built for **Claude Code, OpenAI
Codex, Cursor, and GitHub Copilot**.

> Each skill calls ModelRunner's public MCP server (`mcp.modelrunner.run`) — the same API you can
> use for anything else at **[modelrunner.run »](https://modelrunner.run)**

## What's inside

| Skill | What it does |
|---|---|
| **[`app-store-screenshots`](skills/app-store-screenshots/SKILL.md)** | Turns raw app captures into a cohesive set of App Store / Google Play marketing screenshots — device mockups, on-brand backgrounds, and ASO-aware headlines. Uses the `hakankaan/app-store-screenshot-composer` wrapper and the anchor→style-reference pattern for a consistent set. [Case study »](https://modelrunner.run/blog/generating-app-store-screenshots-with-ai-home-redesign-app-case-study) |
| **[`ios-demo-assets`](skills/ios-demo-assets/SKILL.md)** | Bulk-generates on-spec demo images and loads them into Xcode's `Assets.xcassets` **and** the iOS Simulator's Photos library (`xcrun simctl addmedia`). [Case study »](https://modelrunner.run/blog/generating-demo-assets-for-an-ios-app-with-ai) |

## Prerequisites

Both skills call the **ModelRunner MCP**. Create a ModelRunner account with a small balance at
**[modelrunner.run](https://modelrunner.run)**, then connect the server at `https://mcp.modelrunner.run/mcp`
...
["Agent Skills" open standard](https://modelrunner.run)). A dependency-free build script emits each
...
[MIT](LICENSE) · Built by **[ModelRunner](https://modelrunner.run)** — the unified AI inference API.
